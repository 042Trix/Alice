...
- methodology/M-decide-proactive-board-monitoring.md — v0.1.0 → v0.1.1 (auto-discovery scope; the canonical `BOARDS` allow-list is auto-discovered from `~/.hermes/kanban/boards/` at runtime instead of a hardcoded count). Part 2 rewritten: discovery contract + paired-wiki integrity + drift detector as audit-only safety net. Done-gate check #5 strengthened: verifier must prove auto-discovery works against a newly-added board, not just a hardcoded set. Source: `t_de5e2168` + operator quote 2026-08-11 *"why are we scoping to 9 boards instead of just saying all boards?"*. Companion: paired-wiki on `kanban-board-topology-2026-07.md` (drops "9 active boards" subtitle + "The nine boards" heading; describes the auto-discovery pattern + how to register a new board + drift-detector migration to on-disk-vs-doc-set check). Instance-conformance children filed for coder (the `_stuck_detect_common.py` + `_kanban_boards.py` patches) and verifier (op-guard-17 done-gate).

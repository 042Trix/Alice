...
amended_by_v0_10_3: t_8cd85fb2
amended_by_v0_10_3_source: 'DRY-RUN-5 (`t_20b0f0e6`) + alice-publish audit (`t_ad1fb313`) surfaced a META-with-children deadlock at the kanban dispatcher for the verifier chain (V-2..V-of-V). V-2's claim was rejected with `reason: open_children` because V-3 (V-2''s child) was in `blocked(kind=dependency)`; V-3''s claim was rejected with `reason: verifier_chain_gate` because V-2 (V-3''s parent) had not emitted `compliance: pass`. The two gates formed a permanent cycle. The prior v0.9.3 amend-124 verifier-chain parent-edge gate required every chain member''s parent to satisfy `_verifier_chain_parent_passes`, which checks `parent.status == done AND parent.result starts_with "compliance: pass"`. But the verifier chain has TWO parents per chain member — the chain parent (V-(N-1)) AND the `[MASTER]` close-gate parent. The `[MASTER]` parent was in `blocked(kind=needs_input)` (the HITL #1 operator-disposition window) which failed the gate (status != done AND not the running-master exemption). amend-148 closes the deadlock with TWO complementary exemptions: (1) `_verifier_chain_parent_passes` now returns `True` for `[MASTER]`-prefixed parents in `blocked(kind=needs_input)` (the HITL #1 window) — the verifier chain runs in parallel with the master close-gate per the canonical `per_loop_audit fires in parallel with master; master close-gates via task_links` topology, mirrored for the V-N verifier chain (the chain member''s output IS the master close-gate evidence via task_links); (2) `claim_task`''s `open_children` check now exempts verifier-chain children whose chain parent (V-(N-1)) emitted `compliance: pass` — the verifier chain is sequential V-by-V and a downstream V-(N+1) being open does NOT block V-N''s claim (V-(N+1) cannot emit until V-N does), so V-2..V-of-V can claim despite their own children being open. The exemption is scoped via the title classifier (`_classify_verifier_chain_child` returns `v_n` / `v_of_v` / `retro_a` / `retro_h`) so non-chain children retain strict open_children semantics. The fix lands at TWO sites in `~/.hermes/hermes-agent/hermes_cli/kanban_db.py`: the `_verifier_chain_parent_passes` helper (extends the `[MASTER]` exemption to `blocked(kind=needs_input)`) AND the `claim_task` `open_children` check at line ~7368 (exempts verifier-chain children whose chain parent passes Field-5b). The companion regression suite at `~/.hermes/tests/test_verifier_chain_parent_edge_gate.py` v0.1.0 (NEW; 5 cases — canonical compliant V-2 claim with V-3 open + [MASTER] blocked-needs_input, canonical happy-path V-2 claim with no V-3 yet, [MASTER]-scope guard with non-MASTER blocked-needs_input rejected, Field-5b token gate with V-1 compliance:fail rejected, sequential contract with V-1 still running rejected) confirms both exemptions fire correctly AND the existing constraints are preserved. Source ticket: t_8cd85fb2. Companion dispatcher patch + companion regression suite ship via this ticket (paired-wiki integrity per op-guard-5).'
amended_by_v0_10_3_companion_test: '~/.hermes/tests/test_verifier_chain_parent_edge_gate.py v0.1.0 (NEW; 5-case regression suite covering the amend-148 verifier-chain parent-edge gate exemption at both the _verifier_chain_parent_passes helper AND the claim_task open_children check)'
amended_by_v0_10_3_companion_intent_doc: '~/.hermes/loops/intents/graph-audit-promotion-intent.md v0.10.3 (paired-wiki mirror per op-guard-5; v0.10.3 reflects the verifier-chain parent-edge gate exemption)'
amended_by_x_article_review_v0_2_0: t_003a07e2
amended_by_x_article_review_v0_2_0_source: 'Paired-wiki amendment for `methodology/M-decide-x-article-review-flow.md` v0.1.0 → v0.2.0 (Retro-A classification rubric + auto-accept policy; source `t_003a07e2` per operator 2026-09-05 directive *"also auto accepting retro items"*). The x-article-review flow is the operator-facing sister of the graph-audit-promotion flow (Retro-A + Retro-H vs HITL #1 + HITL #2 + disposition); the v0.2.0 Retro-A classification rubric (cross-link / structural / discipline-preserve classes) mirrors the v0.10.x 4-stage HITL gate-order discipline (per op-guard-32) for the x-article-review Retro-A findings surface. NO contract change to this methodology doc — the paired-wiki amendment preserves op-guard-5 audit trail (this doc cross-references x-article-review Part 4b as a parallel pattern).'
amended_by_v0_10_2: t_4a8bdc0d
amended_by_v0_10_2_source: 'DRY-RUN-5 (`t_20b0f0e6`) + alice-publish audit (`t_ba0ac65f`) surfaced a parent-edge gate defect at the kanban dispatcher: per_loop_audit children of `[MASTER] graph-audit-promotion` parents were rejected with `reason: parents_not_done` when the master was in `blocked(kind=needs_input)` (the canonical HITL #1 operator-disposition window). The pre-amend-147 gate exempted only `running` `[MASTER]` parents (per v0.9.127 amend-134, ticket `t_2034a5ba`) and `blocked(kind=needs_input)` parents for verifier children only (per `t_732e95f4`). The per_loop_audit sits on the canonical parallel-fire-with-master topology (per the `parent-link shape` section below — the master close-gate counts children via `task_links`, the per_loop_audit fires IN PARALLEL with the master via `parents=[]`); the audit PDF IS the operator's decision surface for HITL #1, so the per_loop_audit MUST run concurrently with the master — not wait for the operator to disposition HITL #1. amend-147 codifies the per_loop_audit-child exemption: a per_loop_audit child of a `[MASTER]` graph-audit-promotion parent in `blocked(kind=needs_input)` is allowed to claim and run. The exemption parallels the verifier-child exemption (t_732e95f4) for the same operator-LGTM window reason: the audit output is the operator''s decision surface, not a gate on the parent''s transition to `done`. Scope is limited to canonical graph-audit-promotion masters via the `[MASTER]` title-prefix predicate (mirrors the running-master exemption from amend-134); non-master `blocked(kind=needs_input)` parents remain upstream gates (those are waiting on the operator for a different reason — BA-spec LGTM, paid-spend approval, etc. — and the per_loop_audit running concurrently would defeat the gate). The fix is implemented at the parent-edge gate in `~/.hermes/hermes-agent/hermes_cli/kanban_db.py` at BOTH the `recompute_ready` `eligible_parents` Python check (line ~7066+) AND the `claim_task` SQL `undone` clause (line ~7386+) — the two surfaces MUST mirror each other (verified by the regression suite). Companion test suite: `~/.hermes/tests/test_per_loop_audit_parent_edge_gate.py` v0.1.0 (NEW; 6-case regression suite covering: case_01 [MASTER] blocked(needs_input) → promoted + claimed; case_02 [MASTER] running → promoted + claimed (amend-134 baseline preserved); case_03 non-[MASTER] blocked(needs_input) → rejected (scope guard); case_04 [MASTER] blocked(capability) → rejected (block-kind guard); case_05 Retro-A sibling → rejected (sibling-discrimination guard); case_06 `_is_per_loop_audit_child` classifier unit). The regression suite ships alongside the dispatcher patch via this ticket. Source: `t_4a8bdc0d`.'
amended_by_v0_10_2_companion_test: '~/.hermes/tests/test_per_loop_audit_parent_edge_gate.py v0.1.0 (NEW; 6-case regression suite covering the amend-147 per_loop_audit-child exemption at both the recompute_ready eligible_parents check + the claim_task SQL undone clause + the _is_per_loop_audit_child classifier unit)'
amended_by_v0_10_2_companion_intent_doc: '~/.hermes/loops/intents/graph-audit-promotion-intent.md v0.10.2 (paired-wiki mirror per op-guard-5; v0.10.2 reflects the per_loop_audit-child parent-edge exemption)'
...
amended_by_v0_10_0: t_83916770
amended_by_v0_10_0_source: 'DRY-RUN-5 (`t_20b0f0e6`) surfaced a critical contract gap: the `graph-audit-promotion` flow generates the master-ticket summary PDF for HITL #2 (per amend-91) but does NOT generate a PDF for HITL #1 (intention verification). The HITL #1 surface was `clarify` only — the operator disposed CONFIRM / REVISE / ABANDON without reviewing any artifact. The operator''s prior pushback is documented in `t_c36f239b` body (DRY-RUN-4 archived 2026-08-27): *"this looks like a HITL 2 output. I never saw or approved HITL 1"*. The amend-142 fix enforced gate-order (HITL #1 fires before children dispatch) but did not add a PDF for HITL #1; the dry-run validated the gap. amend-145 codifies the HITL #1 PDF contract — when HITL #1 is ready to fire, the flow auto-generates the master-ticket summary PDF for intention verification at `<workspace_path>/human-review.pdf` BEFORE the `clarify` invocation surfaces the operator-action surface (parallel to amend-91''s HITL #2 PDF). The helper (`~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0, NEW, modeled after `post_graph_audit_disposition_pdf.py` v0.6.4) reads the master ticket body for `target_loop_id: <loop-id>` + the target loop''s paired intent doc + the target loop''s status in `~/.hermes/loops/hermes.yaml` and renders an 8-section Markdown summary (parallel to amend-91''s HITL #2 PDF structure). The flow delivers the PDF to the operator''s Discord DM via `send_dm_attachments` (mandatory, fail-closed per op-guard-3); the master-ticket comment begins with the literal `Graph audit scope/intent ready` prefix (parallel to `Graph audit disposition ready` for HITL #2 per amend-91); the flow appends an audit-line per op-guard-11; the chat-side then fires `clarify` referencing the PDF as evidence (per the executive-reporting skill "PDF-first, then `clarify`" pattern + op-guard-24 + op-guard-28). Both HITL gates MUST generate PDFs to the same canonical contract: HITL #1 (intention verification) via `post_graph_audit_intent_pdf.py` v0.10.0 (NEW) + HITL #2 (disposition) via `post_graph_audit_disposition_pdf.py` v0.6.4 (existing, amend-91). Both PDFs land at `<workspace_path>/human-review.pdf` for the operator-action-dm cron PDF-gate check (per t_24999536 / 03b Part 13). Companion intent doc: `~/.hermes/loops/intents/graph-audit-promotion-intent.md` v0.10.0 (paired-wiki mirror per op-guard-5). Companion helper: `~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0 (NEW, modeled after `post_graph_audit_disposition_pdf.py` v0.6.4; the canonical HITL #1 PDF generation + Discord delivery + vault copy + audit-line pipeline). Companion test suite: `~/.hermes/tests/test_graph_audit_intent_pdf.py` v0.10.0 (NEW, 8-case regression suite parallel to `test_graph_audit_disposition_pdf.py`). Companion env patch: `~/.hermes/profiles/doc-writer/SOUL.md` + `~/.hermes/skills/autonomous-ai-agents/jarvis-coordinator/SKILL.md` (doc-writer ticket body template patched to require the HITL #1 PDF step — parallel to amend-125 SOUL.md patch). Companion dispatcher amend-122 invariant extension: the per_loop_audit doc-writer child inspects the master''s `task_events` for the canonical `[graph-audit-promotion] HITL #1 PDF landed at <workspace_path>/human-review.pdf` comment within 10 seconds of master creation (parallel to amend-122 invariant 5 for the dispatch assertion); missing assertion → `kind=dependency` block + operator-DM cron surface per op-guard-13. Compliance-verifier child ticket: the compliance-verifier (`verifier` profile) runs after all Phase 3 instance children land per op-guard-17 alice-first / instance-second / compliance-gate; the verifier PASS is the done-gate for the graph-audit-promotion v0.10.0 amend-145 promotion. All companion artifacts ship via coder follow-up tickets per op-guard-16 spec-first-flow + op-guard-17 alice-first/instance-second/compliance-gate.'
amended_by_v0_10_0_companion_intent_doc: '~/.hermes/loops/intents/graph-audit-promotion-intent.md v0.10.0 (paired-wiki mirror per op-guard-5; v0.10.0 reflects the HITL #1 PDF contract)'
amended_by_v0_10_0_companion_helper: '~/.hermes/tools/post_graph_audit_intent_pdf.py v0.10.0 (NEW, modeled after post_graph_audit_disposition_pdf.py v0.6.4; the canonical HITL #1 PDF generation + Discord delivery + vault copy + audit-line pipeline)'
amended_by_v0_10_0_companion_test: '~/.hermes/tests/test_graph_audit_intent_pdf.py v0.10.0 (NEW, 8-case regression suite parallel to test_graph_audit_disposition_pdf.py: PDF generated + comment posted + Mermaid renders + idempotency + fail-closed + audit-line + slug derivation + walk-mode discovery)'
amended_by_v0_10_0_companion_env_patch: '~/.hermes/profiles/doc-writer/SOUL.md + ~/.hermes/skills/autonomous-ai-agents/jarvis-coordinator/SKILL.md (doc-writer ticket body template patched to require the HITL #1 PDF step; ships via coder follow-up per op-guard-16)'
amended_by_v0_10_0_companion_dispatcher_patch: '~/.hermes/hermes-agent/hermes_cli/kanban_db.py (amend-122 invariant 5 extension: per_loop_audit child inspects master task_events for the HITL #1 PDF-landed comment within 10 seconds; missing assertion → kind=dependency block + operator-DM cron surface per op-guard-13; ships via coder follow-up per op-guard-21)'
amended_by_v0_10_1: t_91a2a537
amended_by_v0_10_1_source: 'DRY-RUN-5 (`t_20b0f0e6`) criterion 4 — Both HITL PDFs (HITL #1 intention verification + HITL #2 disposition) MUST bundle as appendices into a single canonical findings report artifact (`graph-audit-findings-report.{md,pdf}`) that the V-1 verifier child produces. The amend-145 contract (HITL #1 PDF at `<workspace_path>/human-review.pdf`) + the amend-91 contract (HITL #2 PDF at `~/Documents/HermesVault/0-INBOX/<loop-slug>-master-ticket-summary.pdf`) are independent PDF generators; the amend-146 codifies the **bundling contract**: the V-1 verifier owns the canonical findings report at `<workspace_path>/graph-audit-findings-report.md` (canonical Markdown source) + `<workspace_path>/graph-audit-findings-report.pdf` (operator-facing PDF rendering) that MUST reference both PDFs in `## Appendix A` (HITL #1) + `## Appendix B` (HITL #2) sections. The findings report is the canonical 8-section shape (parallel to `templates/human-digest.md.template` + `03b-decide-operator-agent-interaction.md` Part 13 operator-review-artifact contract): Title + Headline + Verifier findings (per-item scoring + target-loop Mermaid) + Recommended next steps + Evidence anchors + Operator observations + Operator decisions (HITL #1 + HITL #2 decision_refs) + Metadata + Appendix A (HITL #1 PDF) + Appendix B (HITL #2 PDF) + Appendix C (V-1 verifier per-item evidence). The bundler helper `~/.hermes/tools/bundle_graph_audit_findings.py` v0.10.1 (NEW) loads both PDFs + renders the canonical Markdown + renders to PDF via `md_to_pdf.py` + delivers to operator Discord DM via `send_dm_attachments` (mandatory, fail-closed per op-guard-3) + posts master-ticket comment with the literal `Graph audit findings report ready` prefix (parallel to `Graph audit scope/intent ready` + `Graph audit disposition ready`) + copies the PDF to `~/Documents/HermesVault/2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf` for the durable audit trail + appends the canonical audit-line per op-guard-11. Both HITL PDFs MUST be delivered BEFORE the V-1 verifier bundles them — V-1 verifier blocks on `kind=dependency` with the canonical reason `both-pdfs-pending: HITL #1 PDF not delivered` or `both-pdfs-pending: HITL #2 PDF not delivered` if either PDF is missing in `task_events` per the amend-122 invariant 5 extension (parallel pattern: the dispatch assertion). Part 4 step 5b (NEW — V-1 verifier findings-report bundling contract) inserted before Part 4 step 6 (HITL #1 PDF per amend-145). Frontmatter version bumped v0.10.0 → v0.10.1; updated field bumped 2026-08-27T13:55:00Z → 2026-08-27T16:35:00Z. Companion intent doc: `~/.hermes/loops/intents/graph-audit-promotion-intent.md` v0.10.1 (paired-wiki mirror per op-guard-5). Companion template: `~/Documents/alice-framework/templates/graph-audit-findings-report.md.template` v0.1.0 (NEW; the canonical findings-report 8-section + 3-appendix shape contract). Companion helper: `~/.hermes/tools/bundle_graph_audit_findings.py` v0.10.1 (NEW; the canonical V-1 verifier findings-report builder). Companion test: `~/.hermes/tests/test_graph_audit_findings_report.py` v0.10.1 (NEW; 8-case regression suite parallel to test_graph_audit_intent_pdf.py + test_graph_audit_disposition_pdf.py). Companion env patch: `~/.hermes/profiles/verifier/SOUL.md` + `~/.hermes/profiles/doc-writer/SOUL.md` + `~/.hermes/skills/autonomous-ai-agents/jarvis-coordinator/SKILL.md` (ships via coder follow-up per op-guard-16). Companion dispatcher amend-122 invariant 5 extension: V-1 verifier child inspects master task_events for the canonical `Graph audit findings report ready` comment within 10 seconds; missing assertion → kind=dependency block + operator-DM cron surface per op-guard-13 (ships via coder follow-up per op-guard-21). Compliance-verifier child: the verifier runs after all Phase 3 instance children land per op-guard-17 alice-first / instance-second / compliance-gate; the verifier PASS is the done-gate for the graph-audit-promotion v0.10.1 amend-146 promotion. Source: t_91a2a537 (DRY-RUN-5 criterion 4 fix).'
amended_by_v0_10_1_companion_intent_doc: '~/.hermes/loops/intents/graph-audit-promotion-intent.md v0.10.1 (paired-wiki mirror per op-guard-5; v0.10.1 reflects the both-PDFs canonical surface)'
amended_by_v0_10_1_companion_template: '~/Documents/alice-framework/templates/graph-audit-findings-report.md.template v0.1.0 (NEW; the canonical findings-report 8-section + 3-appendix shape contract — bundles Appendix A HITL #1 PDF + Appendix B HITL #2 PDF + Appendix C V-1 verifier per-item evidence)'
amended_by_v0_10_1_companion_helper: '~/.hermes/tools/bundle_graph_audit_findings.py v0.10.1 (NEW; the canonical V-1 verifier findings-report builder that loads both PDFs + renders the canonical Markdown + renders to PDF + delivers via send_dm_attachments fail-closed per op-guard-3 + posts the master-ticket comment with literal Graph audit findings report ready prefix + copies to vault + appends audit-line per op-guard-11)'
amended_by_v0_10_1_companion_test: '~/.hermes/tests/test_graph_audit_findings_report.py v0.10.1 (NEW; 8-case regression suite parallel to test_graph_audit_intent_pdf.py + test_graph_audit_disposition_pdf.py: both PDFs referenced + section ordering + idempotency + fail-closed + audit-line + slug derivation + walk-mode discovery + amend-146 schema)'
amended_by_v0_10_1_companion_env_patch: '~/.hermes/profiles/verifier/SOUL.md + ~/.hermes/profiles/doc-writer/SOUL.md + ~/.hermes/skills/autonomous-ai-agents/jarvis-coordinator/SKILL.md (env patch; ships via coder follow-up per op-guard-16)'
amended_by_v0_10_1_companion_dispatcher_patch: '~/.hermes/hermes-agent/hermes_cli/kanban_db.py (V-1 verifier child inspects master task_events for the canonical Graph audit findings report ready comment within 10 seconds; missing assertion → kind=dependency block + operator-DM cron surface per op-guard-13; ships via coder follow-up per op-guard-21)'
...
5. **Compliance-verifier child** — per op-guard-17, fires per loop as the unnumbered verifier gate (between HITL #1 and HITL #2, per v0.4.0 amend-85) and confirms the instance conforms to the methodology doc + intent + yaml + tests. The verifier's `--result "compliance: pass"` is the gate that flips the YAML on PROMOTE. HITL #2 is BLOCKED until verifier PASS.
   ...
   **5b. V-1 verifier findings-report bundling contract (v0.10.1, amend-146).** The V-1 verifier child MUST produce the canonical findings report that **bundles both HITL PDFs (HITL #1 intention verification + HITL #2 disposition) as appendices** into a single canonical artifact. The bundling contract is the canonical 8-section + 3-appendix shape:

   - **Canonical path:** `<workspace_path>/graph-audit-findings-report.md` (canonical Markdown source) + `<workspace_path>/graph-audit-findings-report.pdf` (operator-facing PDF rendering).
   - **Template:** `~/Documents/alice-framework/templates/graph-audit-findings-report.md.template` v0.1.0 (NEW, this amend; the canonical 8-section + 3-appendix shape contract — Title + Headline + Verifier findings + Recommended next steps + Evidence anchors + Operator observations + Operator decisions + Metadata + Appendix A HITL #1 PDF + Appendix B HITL #2 PDF + Appendix C V-1 verifier per-item evidence).
   - **Helper:** `~/.hermes/tools/bundle_graph_audit_findings.py` v0.10.1 (NEW; the canonical V-1 verifier findings-report builder that loads both PDFs + renders the canonical Markdown + renders to PDF via `md_to_pdf.py` + delivers to operator Discord DM via `send_dm_attachments` (mandatory, fail-closed per op-guard-3) + posts master-ticket comment with the literal `Graph audit findings report ready` prefix (parallel to `Graph audit scope/intent ready` per amend-145 + `Graph audit disposition ready` per amend-91) + copies the PDF to `~/Documents/HermesVault/2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf` for the durable audit trail + appends the canonical audit-line per op-guard-11).
   - **Both PDFs MUST be delivered BEFORE the V-1 verifier bundles them** — the V-1 verifier's first action is to inspect `task_events` for the canonical `Graph audit scope/intent ready` comment (HITL #1 PDF landed, per amend-122 invariant 5 extension) AND the canonical `Graph audit disposition ready` comment (HITL #2 PDF landed). If either PDF is missing, the V-1 verifier blocks on `kind=dependency` with the canonical reason `both-pdfs-pending: HITL #1 PDF not delivered` or `both-pdfs-pending: HITL #2 PDF not delivered` (parallel to the amend-122 invariant 5 dispatch assertion pattern).
   - **The 5-step operator-review-artifact protocol** (per `03b-decide-operator-agent-interaction.md` Part 13): Markdown (canonical, the template) → PDF (rendered via `md_to_pdf.py`) → Discord attachment (mandatory, fail-closed per op-guard-3) → master-ticket comment (literal `Graph audit findings report ready` prefix) → vault copy (`2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf`).
   - **Decision table is mandatory** (per Part 13): the operator MUST fill in the HITL #1 + HITL #2 decision table at section 7; an empty decision is not an acceptance. The decision_refs (`clarify-<ts>-scope-intent` + `clarify-<ts>-disposition`) are recorded in the table.
   - **Idempotency:** a (master-id, target-loop-id) pair that has already produced a findings report at the canonical path is skipped on re-run; `--force-regenerate` overrides the check for verifier E2E.

   The amend-146 contract parallels amend-145 (HITL #1 PDF) and amend-91 (HITL #2 PDF) — all three are auto-generated by the flow itself (NOT chat-side manual creation, per op-guard-21 + the operator's principle *"If the flow is incorrect, fix the flow"*). The V-1 verifier owns the bundling; the findings report is the single canonical operator-review artifact for the audit cycle, with both HITL PDFs as required appendices.

   **Verifier FAIL → auto-file `[Fix]` child ticket to original-profile (amend-88).** When the verifier emits `--result "compliance: fail"` on the per-loop compliance-verifier child, the flow auto-files a `[Fix] [M] [<loop-id>] Address verifier FAIL findings: <short reason>` child ticket assigned to the original profile (default: `doc-writer`); the body carries the FULL verifier FAIL comment (not just the summary), the FAIL summary, the recommended fix path, and the parent verifier ticket link. The auto-filed child is the canonical place for the fix — the operator does NOT manually file a follow-up. After the original-profile commits the fix, the verifier re-fires (3 attempts, 30s/2m/5m); HITL #2 stays BLOCKED until the verifier emits PASS.
...
6. **HITL #1 (Intention verification) PDF generation (v0.10.0, amend-145).** When HITL #1 is ready to fire (after the audit node + the per-loop audit ticket body `## Audit run` section commits per v0.9.4 amend-125), the flow auto-generates the **master-ticket summary PDF for intention verification** via the canonical helper `~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0 (modeled after `post_graph_audit_disposition_pdf.py` v0.6.4 — the structural mirror of amend-91 for HITL #2). The helper:
   1. Resolves the master ticket id + target loop id + the loop slug (from `target_loop_id: <loop-id>` in the master body per v0.2.0 amend-77).
   2. Reads the target loop's paired intent doc (`~/.hermes/loops/intents/<target-loop-id>-intent.md`) + the target loop's entry in `~/.hermes/loops/hermes.yaml` (status, register_with_profile, run_with_profile) — the canonical sources-of-truth per op-guard-17.
   3. Auto-generates an 8-section Markdown summary parallel to amend-91's HITL #2 PDF structure (re-titled for HITL #1 semantics):
      1. Title — the master ticket id + target loop id + operator directive date.
      2. **HITL #1 scope/intent summary** — the target loop's paired intent doc scope + the operator's master-ticket body scope + canonical disposition choices (CONFIRM / REVISE / ABANDON per op-guard-32 v1).
      3. **Target-loop Mermaid diagram** — the loop's flow rendered from the target loop's intent doc OR methodology doc (parallel to amend-92's dynamic extractor). For DRY-RUN placeholder loops with stub intent docs, the diagram falls back to a labeled placeholder per the amend-92 contract.
      4. **Per-loop audit score baseline** — per `loop_live_audit.py` result if available; placeholder `0/6 + 0/1 + 0/1` (all 9 items FAIL — placeholder metadata only) for dry-runs per amend-123 + amend-125.
      5. **HITL chain** — the canonical 2-HITL pattern (HITL #1 → verifier compliance check → HITL #2) with each gate's evidence surface (PDF for HITL #1 + verifier PASS for the unnumbered gate + PDF for HITL #2 — all three are required artifacts before master closes).
      6. **Skip conditions** — per op-guard-29 + op-guard-32 v4.
      7. **Operator review** — the operator disposition surface + how the operator's CONFIRM / REVISE / ABANDON at HITL #1 drives the chain forward (HITL #2 is BLOCKED until the verifier's compliance check PASS lands per v0.4.0 amend-85 + op-guard-32 v3).
      8. **Disposition summary** — the canonical action for the operator at HITL #1 time: confirm the scope/intent is accurate (CONFIRM), or revise the scope/intent + file rewrite tickets (REVISE per v0.4.0 amend-85 — the feedback IS the trigger for Rewrite), or abandon the audit cycle (ABANDON).
   4. Renders the Markdown to PDF at the canonical operator-facing path `<workspace_path>/human-review.pdf` via `md_to_pdf.py`. The path is the canonical HITL gate surface for the operator-action-dm cron PDF-gate check (per t_24999536 / 03b Part 13).
   5. Sends the canonical PDF as a Discord attachment via `send_dm_attachments` — **mandatory, fail-closed per op-guard-3**: a failed send blocks the master on `kind=needs_input` + the chat-side files a `[HUMAN ACTION] graph-audit-intent-pdf-delivery-failed` child ticket to surface the failure to the operator-DM cron. The chat-side MUST NOT fire the HITL #1 `clarify` before Discord delivery succeeds (parallel to the v0.6.0 amend-91 fail-closed contract for HITL #2).
   6. Posts a master-ticket comment that begins exactly with the literal `Graph audit scope/intent ready` prefix (parallel to `Graph audit disposition ready` for HITL #2 per amend-91).
   7. Copies the PDF to the durable audit trail at `~/Documents/HermesVault/2-ATOMIC/decisions/<loop-slug>-graph-audit-intent.pdf`.
   8. Appends a one-line audit-line to `~/Documents/HermesVault/log.md` per op-guard-11 via `vault_log.append_audit_line`.
   9. The chat-side then fires `clarify` referencing the PDF as evidence (per the executive-reporting skill "PDF-first, then `clarify`" pattern + op-guard-24 + op-guard-28). The operator's HITL #1 disposition (CONFIRM / REVISE / ABANDON) is captured as the `clarify-<timestamp>-scope-intent` decision_ref per op-guard-29 + op-guard-10 v2 §1.2.

   **Pair with HITL #2 PDF (amend-91).** Both HITL gates MUST generate PDFs to the same canonical contract:
   - HITL #1 (Intention verification) — via `post_graph_audit_intent_pdf.py` v0.10.0 (NEW, this amend).
   - HITL #2 (Disposition) — via `post_graph_audit_disposition_pdf.py` v0.6.4 (existing, amend-91).
   - Both PDFs land at `<workspace_path>/human-review.pdf` for the operator-action-dm cron PDF-gate check (per t_24999536 / 03b Part 13).

   **Why this amendment (DRY-RUN-5 contract gap).** DRY-RUN-5 (`t_20b0f0e6`) was gate-blocked at HITL #1 with no PDF delivered; the workspace `~/.hermes/kanban/boards/loop-builder/workspaces/t_20b0f0e6/` was empty — no `human-review.pdf`. Per op-guard-3 fail-closed contract for operator-action items, the PDF must be a real artifact at delivery time, not a deferred clarification. The operator's prior pushback in `t_c36f239b` body (DRY-RUN-4 archived 2026-08-27, verbatim: *"this looks like a HITL 2 output. I never saw or approved HITL 1"*) shows this is a structural defect, not a one-off. The amend-142 fix enforced the gate-order (HITL #1 fires before children dispatch) but did not add a PDF for HITL #1; the dry-run validated the gap. amend-145 closes it. The companion dispatcher amend-122 invariant 5 extension asserts the HITL #1 PDF comment landed in `task_events` within 10 seconds of master creation (parallel to the dispatcher-fired assertion).
...
8. **Action node** — flips YAML `status:` + bumps intent doc `version:` + appends audit-line per the operator's HITL #2 disposition. Per op-guard-30 evidence + compliance-verifier chain.
...
## v0.10.0 amendment record (2026-08-27, source: t_83916770, amend-145)

The v0.10.0 amendment codifies the **HITL #1 (Intention verification) PDF contract** per the DRY-RUN-5 (`t_20b0f0e6`) finding + the operator's prior pushback in `t_c36f239b` body (DRY-RUN-4 archived 2026-08-27, verbatim: *"this looks like a HITL 2 output. I never saw or approved HITL 1"*).

### What changed

1. **Part 4 step 6 (NEW) — HITL #1 (Intention verification) PDF generation.** When HITL #1 is ready to fire (after the audit node + the per-loop audit ticket body `## Audit run` section commits per v0.9.4 amend-125), the flow auto-generates the master-ticket summary PDF for intention verification via the canonical helper `~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0. The helper follows the exact 9-step protocol from amend-91 (HITL #2 PDF): resolve ids → read intent doc + yaml entry → auto-generate 8-section Markdown → render to `<workspace_path>/human-review.pdf` via `md_to_pdf.py` → Discord delivery via `send_dm_attachments` (mandatory, fail-closed per op-guard-3) → post master-ticket comment with `Graph audit scope/intent ready` prefix → vault copy at `~/Documents/HermesVault/2-ATOMIC/decisions/<loop-slug>-graph-audit-intent.pdf` → audit-line per op-guard-11 → chat-side fires `clarify` referencing the PDF as evidence.

2. **Part 4 step 7 (renumbered from step 6) — HITL #2 (Disposition) PDF generation (v0.6.0 amend-91).** Step 6 of v0.6.0 (HITL #2 PDF) is renumbered to step 7. Body unchanged — same canonical helper `~/.hermes/tools/post_graph_audit_disposition_pdf.py` v0.6.4, same `Graph audit disposition ready` prefix, same vault copy at `2-ATOMIC/decisions/<loop-slug>-master-ticket-summary.pdf`.

3. **Part 4 step 8 (renumbered from step 7) — Action node.** Unchanged; renumbered only.

4. **Frontmatter** version bumped v0.9.4 → v0.10.0; `updated:` field bumped 2026-08-26T16:05Z → 2026-08-27T13:55:00Z; `amended_by_v0_10_0` added with source t_83916770; `amended_by_v0_10_0_source` added with the amend-145 contract summary; 5 new companion fields added (`amended_by_v0_10_0_companion_intent_doc`, `_companion_helper`, `_companion_test`, `_companion_env_patch`, `_companion_dispatcher_patch`).

### Why this amendment

DRY-RUN-5 (`t_20b0f0e6`) was gate-blocked at HITL #1 with no PDF delivered. The workspace `~/.hermes/kanban/boards/loop-builder/workspaces/t_20b0f0e6/` was empty — no `human-review.pdf`. Per op-guard-3 fail-closed contract for operator-action items, the PDF must be a real artifact at delivery time, not a deferred clarification. The operator's prior pushback in `t_c36f239b` body (DRY-RUN-4 archived 2026-08-27, verbatim: *"this looks like a HITL 2 output. I never saw or approved HITL 1"*) shows this is a structural defect, not a one-off. The amend-142 fix enforced gate-order (HITL #1 fires before children dispatch) but did not add a PDF for HITL #1; the dry-run validated the gap. amend-145 closes it.

The amend-145 contract mirrors amend-91 for HITL #2 (the disposition PDF contract). Both HITL gates MUST generate PDFs to the same canonical contract:
- HITL #1 (Intention verification) — via `post_graph_audit_intent_pdf.py` v0.10.0 (NEW, this amend).
- HITL #2 (Disposition) — via `post_graph_audit_disposition_pdf.py` v0.6.4 (existing, amend-91).
- Both PDFs land at `<workspace_path>/human-review.pdf` for the operator-action-dm cron PDF-gate check (per t_24999536 / 03b Part 13).

The companion dispatcher amend-122 invariant 5 extension asserts the HITL #1 PDF comment landed in `task_events` within 10 seconds of master creation (parallel to the dispatcher-fired assertion). Missing assertion → `kind=dependency` block + operator-DM cron surface per op-guard-13. The chat-side MUST NOT fire the HITL #1 `clarify` before Discord delivery succeeds (parallel to the v0.6.0 amend-91 fail-closed contract for HITL #2).

### Forward compatibility

Per-loop tickets filed under v0.9.4 (or any prior version) are grandfathered. The amend-145 contract applies forward from this ticket's ship date (2026-08-27). DRY-RUN-5 (`t_20b0f0e6`) retests as DRY-RUN-6 once the instance-side patches land + the compliance-verifier confirms.

### Companion artifacts (ships via coder follow-up per op-guard-16 + op-guard-17)

- **Companion intent doc:** `~/.hermes/loops/intents/graph-audit-promotion-intent.md` v0.10.0 (paired-wiki mirror per op-guard-5; v0.10.0 reflects the HITL #1 PDF contract).
- **Companion concept note:** `~/Documents/HermesVault/2-ATOMIC/concepts/graph-audit-promotion-hitl-1-pdf-contract-v0-10-0-2026-08-27.md` (NEW; the canonical concept note for amend-145).
- **Companion helper:** `~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0 (NEW, modeled after `post_graph_audit_disposition_pdf.py` v0.6.4 — the canonical HITL #1 PDF generation + Discord delivery + vault copy + audit-line pipeline).
- **Companion test suite:** `~/.hermes/tests/test_graph_audit_intent_pdf.py` v0.10.0 (NEW, 8-case regression suite parallel to `test_graph_audit_disposition_pdf.py`: PDF generated + comment posted + Mermaid renders + idempotency + fail-closed + audit-line + slug derivation + walk-mode discovery).
- **Companion env patch:** `~/.hermes/profiles/doc-writer/SOUL.md` + `~/.hermes/skills/autonomous-ai-agents/jarvis-coordinator/SKILL.md` (doc-writer ticket body template patched to require the HITL #1 PDF step — parallel to the amend-125 SOUL.md patch).
- **Companion dispatcher amend-122 invariant 5 extension:** `~/.hermes/hermes-agent/hermes_cli/kanban_db.py` — the per_loop_audit doc-writer child inspects the master's `task_events` for the canonical `[graph-audit-promotion] HITL #1 PDF landed at <workspace_path>/human-review.pdf` comment within 10 seconds of master creation (parallel to amend-122 invariant 5 for the dispatch assertion). Missing assertion → `kind=dependency` block + operator-DM cron surface per op-guard-13.
- **Compliance-verifier child ticket:** the compliance-verifier (`verifier` profile) runs after all Phase 3 instance children land per op-guard-17 alice-first / instance-second / compliance-gate; the verifier PASS is the done-gate for the graph-audit-promotion v0.10.0 amend-145 promotion.

### Audit-line per op-guard-11

```
## [2026-08-27T13:55:00Z] doc-writer-shipped — methodology/M-decide-graph-audit-promotion-flow.md v0.9.4 → v0.10.0 (amend-145, source t_83916770). HITL #1 (Intention verification) PDF contract codified: when HITL #1 is ready to fire, the flow auto-generates the master-ticket summary PDF for intention verification at `<workspace_path>/human-review.pdf` BEFORE the `clarify` invocation surfaces the operator-action surface (parallel to amend-91's HITL #2 PDF). Helper `~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0 (NEW, modeled after `post_graph_audit_disposition_pdf.py` v0.6.4) reads master body for `target_loop_id: <loop-id>` + target loop's paired intent doc + target loop's yaml entry and renders an 8-section Markdown summary. Discord delivery via `send_dm_attachments` (mandatory, fail-closed per op-guard-3); master-ticket comment begins with literal `Graph audit scope/intent ready` prefix; vault copy at `2-ATOMIC/decisions/<loop-slug>-graph-audit-intent.pdf`; audit-line per op-guard-11; chat-side then fires `clarify` referencing the PDF as evidence. Both HITL gates MUST generate PDFs to the same canonical contract: HITL #1 via `post_graph_audit_intent_pdf.py` v0.10.0 (NEW) + HITL #2 via `post_graph_audit_disposition_pdf.py` v0.6.4 (existing, amend-91); both PDFs land at `<workspace_path>/human-review.pdf` for the operator-action-dm cron PDF-gate check. Companion dispatcher amend-122 invariant 5 extension asserts the HITL #1 PDF comment landed in `task_events` within 10 seconds of master creation; missing assertion → `kind=dependency` block + operator-DM cron surface per op-guard-13. Part 4 step 6 (NEW — HITL #1 PDF contract) inserted; existing Part 4 step 6 (HITL #2 PDF, amend-91) renumbered to step 7; existing Part 4 step 7 (Action node) renumbered to step 8. Frontmatter version bumped v0.9.4 → v0.10.0; updated: field bumped 2026-08-26T16:05Z → 2026-08-27T13:55:00Z; amended_by_v0_10_0 + amended_by_v0_10_0_source + 5 new companion fields added. Coder follow-up tickets (per op-guard-16) ship: (1) intent doc v0.10.0 paired-wiki mirror, (2) companion concept note, (3) `~/.hermes/tools/post_graph_audit_intent_pdf.py` v0.10.0 helper, (4) `~/.hermes/tests/test_graph_audit_intent_pdf.py` v0.10.0 8-case regression suite, (5) `~/.hermes/profiles/doc-writer/SOUL.md` + jarvis-coordinator SKILL.md env patch (require the HITL #1 PDF step), (6) `~/.hermes/hermes-agent/hermes_cli/kanban_db.py` dispatcher amend-122 invariant 5 extension. Compliance-verifier child is the done-gate per op-guard-17. Source: DRY-RUN-5 t_20b0f0e6 + DRY-RUN-4 t_c36f239b operator pushback (verbatim: "this looks like a HITL 2 output. I never saw or approved HITL 1").
```

---

## [2026-08-27T13:55:00Z] doc-writer-shipped — methodology/M-decide-graph-audit-promotion-flow.md v0.9.4 → v0.10.0 (amend-145, source t_83916770). HITL #1 PDF contract codified. See v0.10.0 amendment record section above for full detail.

---

## v0.10.1 amendment record (2026-08-27, source: t_91a2a537, amend-146)

The v0.10.1 amendment codifies the **V-1 verifier findings-report bundling contract** per the DRY-RUN-5 (`t_20b0f0e6`) criterion 4 finding — both HITL PDFs (HITL #1 intention verification per amend-145 + HITL #2 disposition per amend-91) MUST bundle as appendices into a single canonical findings report artifact that the V-1 verifier child produces.

### What changed

1. **Part 4 step 5b (NEW) — V-1 verifier findings-report bundling contract (v0.10.1, amend-146).** The V-1 verifier child MUST produce the canonical findings report that **bundles both HITL PDFs as appendices** into a single canonical artifact. The bundling contract is the canonical 8-section + 3-appendix shape (parallel to `templates/human-digest.md.template` 8-section shape + `03b-decide-operator-agent-interaction.md` Part 13 operator-review-artifact contract):
   - **Canonical path:** `<workspace_path>/graph-audit-findings-report.md` (canonical Markdown source) + `<workspace_path>/graph-audit-findings-report.pdf` (operator-facing PDF rendering).
   - **Template:** `~/Documents/alice-framework/templates/graph-audit-findings-report.md.template` v0.1.0 (NEW; the canonical 8-section + 3-appendix shape contract — Title + Headline + Verifier findings + Recommended next steps + Evidence anchors + Operator observations + Operator decisions + Metadata + Appendix A HITL #1 PDF + Appendix B HITL #2 PDF + Appendix C V-1 verifier per-item evidence).
   - **Helper:** `~/.hermes/tools/bundle_graph_audit_findings.py` v0.10.1 (NEW; the canonical V-1 verifier findings-report builder that loads both PDFs + renders the canonical Markdown + renders to PDF via `md_to_pdf.py` + delivers to operator Discord DM via `send_dm_attachments` (mandatory, fail-closed per op-guard-3) + posts master-ticket comment with the literal `Graph audit findings report ready` prefix + copies the PDF to `~/Documents/HermesVault/2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf` for the durable audit trail + appends the canonical audit-line per op-guard-11).
   - **Both PDFs MUST be delivered BEFORE the V-1 verifier bundles them** — the V-1 verifier's first action is to inspect `task_events` for the canonical `Graph audit scope/intent ready` comment (HITL #1 PDF landed, per amend-122 invariant 5 extension) AND the canonical `Graph audit disposition ready` comment (HITL #2 PDF landed). If either PDF is missing, the V-1 verifier blocks on `kind=dependency` with the canonical reason `both-pdfs-pending: HITL #1 PDF not delivered` or `both-pdfs-pending: HITL #2 PDF not delivered` (parallel to the amend-122 invariant 5 dispatch assertion pattern).
   - **The 5-step operator-review-artifact protocol** (per `03b-decide-operator-agent-interaction.md` Part 13): Markdown (canonical, the template) → PDF (rendered via `md_to_pdf.py`) → Discord attachment (mandatory, fail-closed per op-guard-3) → master-ticket comment (literal `Graph audit findings report ready` prefix) → vault copy (`2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf`).
   - **Decision table is mandatory** (per Part 13): the operator MUST fill in the HITL #1 + HITL #2 decision table at section 7; an empty decision is not an acceptance. The decision_refs (`clarify-<ts>-scope-intent` + `clarify-<ts>-disposition`) are recorded in the table.
   - **Idempotency:** a (master-id, target-loop-id) pair that has already produced a findings report at the canonical path is skipped on re-run; `--force-regenerate` overrides the check for verifier E2E.

2. **Frontmatter** version bumped v0.10.0 → v0.10.1; `updated:` field bumped 2026-08-27T13:55:00Z → 2026-08-27T16:35:00Z; `amended_by_v0_10_1` added with source t_91a2a537; `amended_by_v0_10_1_source` added with the bundling contract summary; 7 new companion fields added (`amended_by_v0_10_1_companion_intent_doc`, `_companion_template`, `_companion_helper`, `_companion_test`, `_companion_env_patch`, `_companion_dispatcher_patch`).

### Why this amendment

DRY-RUN-5 (`t_20b0f0e6`) criterion 4 required both HITL PDFs to generate to spec — the HITL #1 PDF helper was missing per amend-145 (gap from t_c36f239b); the HITL #2 PDF helper exists per amend-91 but had not been exercised end-to-end against the DRY-RUN-5 fixture. The amend-145 contract (HITL #1 PDF at `<workspace_path>/human-review.pdf`) + the amend-91 contract (HITL #2 PDF at `~/Documents/HermesVault/0-INBOX/<loop-slug>-master-ticket-summary.pdf`) are independent PDF generators; neither produces a single canonical operator-review artifact that bundles both. The operator's review surface (Discord DM) needs the **findings report** with both PDFs as appendices — the audit cycle's single operator-review artifact, parallel to the x-article-review Phase 4 council verdict (per the v0.10.0 amend-91 reference: *"I am not going to review a manually created PDF. It needs to be done by the flow itself. If the flow is incorrect, fix the flow"*).

The amend-146 codifies the durable fix: the V-1 verifier child produces the canonical `graph-audit-findings-report.{md,pdf}` artifact that bundles both PDFs as appendices. The operator reviews the findings report (NOT two separate PDFs in chat); the V-1 verifier owns the bundling; the canonical shape is the 8-section + 3-appendix template (Title + Headline + Verifier findings + Recommended next steps + Evidence anchors + Operator observations + Operator decisions + Metadata + Appendix A HITL #1 PDF + Appendix B HITL #2 PDF + Appendix C V-1 verifier per-item evidence). The amend-122 invariant 5 extension (V-1 verifier inspects `task_events` for both PDF-landed comments) is the canonical gate that prevents the bundler from running on incomplete data.

### Forward compatibility

Per-loop tickets filed under v0.10.0 (or any prior version) are grandfathered. The amend-146 contract applies forward from this ticket's ship date (2026-08-27). DRY-RUN-5 (`t_20b0f0e6`) retests as DRY-RUN-6 once the instance-side patches land + the compliance-verifier confirms.

### Companion artifacts (ships via coder follow-up per op-guard-16 + op-guard-17)

- **Companion intent doc:** `~/.hermes/loops/intents/graph-audit-promotion-intent.md` v0.10.1 (paired-wiki mirror per op-guard-5; v0.10.1 reflects the both-PDFs canonical surface).
- **Companion concept note:** `~/Documents/HermesVault/2-ATOMIC/concepts/graph-audit-promotion-both-pdfs-canonical-surface-v0-10-1-2026-08-27.md` (NEW; the canonical concept note for amend-146).
- **Companion template:** `~/Documents/alice-framework/templates/graph-audit-findings-report.md.template` v0.1.0 (NEW; the canonical 8-section + 3-appendix shape contract — bundles Appendix A HITL #1 PDF + Appendix B HITL #2 PDF + Appendix C V-1 verifier per-item evidence). Ships via this doc-writer ticket (phase 1+2).
- **Companion helper:** `~/.hermes/tools/bundle_graph_audit_findings.py` v0.10.1 (NEW; the canonical V-1 verifier findings-report builder that loads both PDFs + renders the canonical Markdown + renders to PDF + delivers via `send_dm_attachments` fail-closed per op-guard-3 + posts the master-ticket comment with literal `Graph audit findings report ready` prefix + copies to vault + appends audit-line per op-guard-11). Ships via coder follow-up.
- **Companion test suite:** `~/.hermes/tests/test_graph_audit_findings_report.py` v0.10.1 (NEW; 8-case regression suite parallel to `test_graph_audit_intent_pdf.py` + `test_graph_audit_disposition_pdf.py`: both PDFs referenced + section ordering + idempotency + fail-closed + audit-line + slug derivation + walk-mode discovery + amend-146 schema). Ships via coder follow-up.
- **Companion env patch:** `~/.hermes/profiles/verifier/SOUL.md` + `~/.hermes/profiles/doc-writer/SOUL.md` + `~/.hermes/skills/autonomous-ai-agents/jarvis-coordinator/SKILL.md` (env patch; ships via coder follow-up per op-guard-16).
- **Companion dispatcher amend-122 invariant 5 extension:** `~/.hermes/hermes-agent/hermes_cli/kanban_db.py` — V-1 verifier child inspects master `task_events` for the canonical `Graph audit findings report ready` comment within 10 seconds of V-1 dispatch (parallel to amend-122 invariant 5 for the dispatch assertion). Missing assertion → `kind=dependency` block + operator-DM cron surface per op-guard-13. Ships via coder follow-up per op-guard-21.
- **Compliance-verifier child ticket:** the compliance-verifier (`verifier` profile) runs after all Phase 3 instance children land per op-guard-17 alice-first / instance-second / compliance-gate; the verifier PASS is the done-gate for the graph-audit-promotion v0.10.1 amend-146 promotion.

### Audit-line per op-guard-11

```
## [2026-08-27T16:35:00Z] doc-writer-shipped — methodology/M-decide-graph-audit-promotion-flow.md v0.10.0 → v0.10.1 (amend-146, source t_91a2a537). V-1 verifier findings-report bundling contract codified: V-1 verifier child produces the canonical graph-audit-findings-report.{md,pdf} artifact that bundles HITL #1 PDF (per amend-145) + HITL #2 PDF (per amend-91) as appendices (Appendix A + Appendix B respectively) plus the V-1 verifier per-item evidence (Appendix C). Canonical template at ~/Documents/alice-framework/templates/graph-audit-findings-report.md.template v0.1.0 (NEW; the 8-section + 3-appendix shape contract — parallel to templates/human-digest.md.template + 03b Part 13 operator-review-artifact contract). Canonical helper at ~/.hermes/tools/bundle_graph_audit_findings.py v0.10.1 (NEW; loads both PDFs + renders canonical Markdown + renders to PDF via md_to_pdf.py + delivers to operator Discord DM via send_dm_attachments fail-closed per op-guard-3 + posts master-ticket comment with literal Graph audit findings report ready prefix + copies PDF to 2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf + appends audit-line per op-guard-11). Both PDFs MUST be delivered BEFORE the V-1 verifier bundles them — V-1 verifier inspects task_events for canonical Graph audit scope/intent ready + Graph audit disposition ready comments per amend-122 invariant 5 extension; missing assertion → kind=dependency block with canonical reason both-pdfs-pending: HITL #1 PDF not delivered OR both-pdfs-pending: HITL #2 PDF not delivered. The 5-step operator-review-artifact protocol (per 03b Part 13): Markdown canonical → PDF rendered → Discord attachment mandatory fail-closed per op-guard-3 → master-ticket comment with literal Graph audit findings report ready prefix → vault copy at 2-ATOMIC/decisions/<loop-slug>-graph-audit-findings-report.pdf. Decision table at section 7 is mandatory per Part 13 — operator MUST fill in HITL #1 + HITL #2 decision_refs (clarify-<ts>-scope-intent + clarify-<ts>-disposition); empty decision is not an acceptance. Idempotency: (master-id, target-loop-id) pair already produced findings report at canonical path → skipped on re-run; --force-regenerate overrides the check for verifier E2E. Part 4 step 5b (NEW — V-1 verifier findings-report bundling contract) inserted inside the existing Part 4 step 5 (Compliance-verifier child) block. Frontmatter version bumped v0.10.0 → v0.10.1; updated field bumped 2026-08-27T13:55:00Z → 2026-08-27T16:35:00Z; amended_by_v0_10_1 + amended_by_v0_10_1_source + 7 new companion fields added. Coder follow-up tickets (per op-guard-16) ship: (1) intent doc v0.10.1 paired-wiki mirror, (2) companion concept note, (3) ~/.hermes/tools/bundle_graph_audit_findings.py v0.10.1 helper, (4) ~/.hermes/tests/test_graph_audit_findings_report.py v0.10.1 8-case regression suite, (5) ~/.hermes/profiles/verifier/SOUL.md + ~/.hermes/profiles/doc-writer/SOUL.md + jarvis-coordinator SKILL.md env patch, (6) ~/.hermes/hermes-agent/hermes_cli/kanban_db.py dispatcher amend-122 invariant 5 extension for the findings-report-landed assertion. Compliance-verifier child is the done-gate per op-guard-17. Source: DRY-RUN-5 t_20b0f0e6 criterion 4 + DRY-RUN-4 t_c36f239b operator pushback (verbatim: "this looks like a HITL 2 output. I never saw or approved HITL 1"). Paired-wiki integrity per op-guard-5: methodology doc v0.10.1 + intent doc v0.10.1 + companion concept note ship in the same change set. Source: t_91a2a537 (DRY-RUN-5 criterion 4 fix).
```

---

## [2026-08-27T16:35:00Z] doc-writer-shipped — methodology/M-decide-graph-audit-promotion-flow.md v0.10.0 → v0.10.1 (amend-146, source t_91a2a537). V-1 verifier findings-report bundling contract codified. See v0.10.1 amendment record section above for full detail.
